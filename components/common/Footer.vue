<template>
  <div class="i_footer">
    <div class="footer_content clearfix" v-if="list">
      <div class="footer_msg" v-for="(item, index) in list" :key="index">
        <ul>
          ...
          <li class="m_row"></li>
          <li v-for="(that, num) in item.websiteNavList" :key="num">
            <a :href="clientData.mainUrl+'/terrace/'+that.id">{{that.navName}}</a>
          </li>
        </ul>
      </div>
    ...
    <div class="icp_num">
      <a href="http://www.roncoo.net/" target="_blank" class="lingke_link">黑曼巴教育云</a> 提供技术支持
      ...
      <a
        :href="'http://www.beian.gov.cn/portal/registerSystemInfo?recordcode=' + service.prnNo"
        target="_blank"
        v-if="service.prn"
        class="lingke_link"
      >
        <img src="~/assets/image/prn_icon.png" class="prn_icon" alt>
        &nbsp;{{service.prn}}
      </a>
    </div>
  </div>
